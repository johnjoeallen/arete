 ...
 * META-INF/services/net.dublinux.arete.scoring.spi.SpecScoringPlugin}
 ...
 *       {@link #score(SpecInput)}.</li>
 *   <li>{@link #score(SpecInput)} may then be called any number of
 ...
 * {@link #score(SpecInput)} invocations after {@link #configure(Map)}
 * has completed. In practice this means: don't mutate instance fields
 * inside {@code score()}, and if the wrapped engine has its own
 ...
 * {@code score()} does not declare any checked exceptions — see {@link
 * ScoringResult} for why. Implementations should catch their own
 * unexpected failures and return {@link ScoringResult#pluginError}
 ...
 */
public interface SpecScoringPlugin {
     ...
     * to a config key it happens to call "scoring type" internally) is
     ...
     * failure modes — see {@link ScoringResult}.
     *
     * @param input never null
     * @return never null
     */
    ScoringResult score(SpecInput input);
}
