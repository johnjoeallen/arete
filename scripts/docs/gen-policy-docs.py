...
lines = [
    ...
    f"**{len(manifest['detectors'])} detectors**.",
    "",
...
for cat in sorted(by_cat):
    lines += [f"## {cat}", ""]
    lines.append("| Rule | Title | Detector | " + " | ".join(short[p] for p in policy_ids) + " |")
    ...
    for r in sorted(by_cat[cat], key=lambda x: x["id"]):
        ...
        lines.append(f"| `{r['id']}` | {title_tail(r['title'])} | `{r['detector']}` | " + " | ".join(cells) + " |")
    lines.append("")
