# Starlark rule port (issue #125)

Status: **all 22 rules ported and parity-verified**

Realises the [research note](policy-engine-dsl-research.md): every bundled
rule, rewritten in Starlark + RE2/J, produces **identical** diagnostics to
...
- **Parity: all 22 rules, 0 mismatches.** The sweep runs >900 (spec ×
  policy × rule) comparisons plus a per-rule pass. Every rule except the
  two intentionally-empty ones (`manual`, `compatibility`) is exercised
  non-vacuously — the test fails if any rule is only checked on empty
  output.
...
- **Safe by construction** — rejected as language properties, not filtered
  ...
  `1 // 0` (→ `RuleException`), `for i in range(1e8)` (step cap).
...
- **Groovy is supported but disabled by default** (not deprecated) — it is
  ...
  first-class option. Opt back in with `rule-language=groovy` (config) or
  `-Darete.policy.rule-language=groovy`; the plugin logs a warning.
  `PolicyBundleLoader.LoadOptions(forceGroovy)` drives it. A rule missing
  its `.star` falls back to Groovy with a per-rule warning.
- `StarlarkMatcherEvaluator` moved into `net.dublinux.arete.validation.policy` and
  returns `List<Diagnostic>` / throws `RuleException` like the Groovy one.
