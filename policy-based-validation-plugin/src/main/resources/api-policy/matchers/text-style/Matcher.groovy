 ...
 * map. It returns diagnostic maps only: score, policy disposition, and
 ...
 * values rather than Swagger/OpenAPI parser objects, so rule code does
 ...
 *     guessed; a later rule revision can add an explicit exception list.
 *
 ...
 * closed (no accidental fabricated diagnostic).
 */
{ Map api, Map rule ->
    ...
    def matches = { operation ->
        ...
        if (parameters.match == 'non-action-oriented' && actionVerbs.any { verb -> summary.startsWith(verb) }) return false
        true
    }
