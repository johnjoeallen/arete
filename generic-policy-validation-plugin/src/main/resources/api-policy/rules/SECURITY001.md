...
The rule has `operation` scope and uses the `security` rule. For each
operation, an operation-level `security` declaration is used when present;
otherwise the document-level declaration is inherited. The rule reports
...
can require particular scopes. The rule does not inspect the security
