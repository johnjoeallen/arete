...
When `maximum-depth` is present, the detector counts non-parameter path
...
The `maximum: 8` value is not used by this detector branch.
...
`maximum-depth: 2` controls this rule; the detector also supports a separate
