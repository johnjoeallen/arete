...
The rule lowercases each path and summary and treats an operation as
...
rule does not inspect the request body, so `payload: collection` documents
...
not match the rule’s conditions.
...
and create/bulk text currently affect matching. The rule does not prove
