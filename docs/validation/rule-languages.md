...
Every bundled rule ships in three languages: **Distill** (`Matcher.dsl`, the
default runtime — see the [Distill reference](distill.md)), **Starlark**
(`Matcher.star`, the sandboxed fallback), and **Groovy** (`Matcher.groovy`,
...
operations, retain the matching operations, and create diagnostics.

## The same rule in three languages
...
Groovy is expressive and close to the original rule implementations, but
...
fallback runtime, kept parity-equivalent to Distill.
...
Distill keeps Java-shaped operators while using Groovy-like trailing closures for
collection pipelines. `expand` names the operation that turns each path into
its operation collection; it is the Distill equivalent of Groovy `collectMany`
and Starlark's nested loop. The [Distill reference](distill.md) is the full guide.

For regular expressions Distill borrows Groovy's slashy literal and match
...
| Operation | Groovy | Starlark | Distill |
|---|---|---|---|
...
| Create a finding | `[pointer: ..., path: ..., message: ...]` | `{"pointer": ..., "path": ..., "message": ...}` | `diagnostic(pointer, path, message)` |
| Regular expressions | `/…/` with `==~` / `=~` | `re_fullmatch(...)` | `/…/` with `==~` / `=~` (or `regexFullMatch(...)`) |
| Entry point | closure `{ Map api, Map rule -> ... }` | `detect(api, rule)` | `distill(api, rule) { ... }` |

## Choosing a language

**Distill is the default** for bundled rules: it is sandboxed, and its small
expression grammar keeps rules terse and reviewable. Starlark is the
sandboxed fallback, used for any rule that ships only a `Matcher.star`
...
[policy engine guide](policy-engine.md#rule-languages); the full Distill
grammar and builtin catalogue are in the [Distill reference](distill.md).

Whichever language a bundle picks, the findings are the same: `DistillParityTest`
pins every `Matcher.dsl` to its `Matcher.star`, and `GroovyStarlarkParityTest`
sweeps every bundle rule against both other runtimes so a `Matcher.groovy` can
