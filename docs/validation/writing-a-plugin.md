...
[`SpecValidationPlugin`](https://github.com/johnjoeallen/speculate/blob/main/speculate-validation-spi/src/main/java/net/dublinux/speculate/validation/spi/SpecValidationPlugin.java)
from the `speculate-validation-spi` module and registers itself via
`ServiceLoader` — no Speculate-specific base class or annotations required.

!!! tip "Prefer the policy bundle for rule changes"
    If you just want different **rules**, you probably don't need a new plugin
    at all. The bundled [Speculate Policy Engine](policy-engine.md) is
    driven entirely by text files — add a rule, detector, or policy by editing
    Markdown/YAML/Starlark. Write a `SpecValidationPlugin` only when you need a
    different **engine**.
...
`net.dublinux.speculate:speculate-validation-spi`. It isn't always the latest
`speculate` release tag: publishing to Central is a separate, manually-triggered
...
Then drop the jar into `~/.speculate/plugins` and restart.

## Packaging

Speculate loads each plugin through an **isolated, single-jar**
...
between plugins — and between a plugin and Speculate itself — never collide.

## A worked example

The bundled
[`generic-policy-validation-plugin`](https://github.com/johnjoeallen/speculate/tree/main/generic-policy-validation-plugin)
