...
The configured header list may be overridden per policy. The rule checks
the documented OpenAPI response only and does not inspect runtime headers.

## Detection and scope

The rule has `response` scope and uses the `response-header` rule. Its
...
more configured headers produces an diagnostic at the containing operation
pointer. Other response statuses are ignored.

## Configuration and limitations

Policies may override the comma-separated `headers` list, `status`, or
required mode. The rule checks only declared response headers and does not
