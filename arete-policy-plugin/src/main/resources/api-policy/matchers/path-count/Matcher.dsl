distill(api, rule) {
    return rule.parameters.keys.any { k -> k == "maximum-depth" }
        ? api.paths
            .filter { path -> count(pathSegments(path.path)) > rule.parameters["maximum-depth"] }
        ...
        ? api.paths
            .filter { path -> count(pathSegments(path.path)) > 1
                && !distinct(api.paths.map { other ->
                        count(pathSegments(other.path)) > 0 ? pathSegments(other.path)[0] : null })
                    .any { root -> root == last(pathSegments(path.path)) } }
            .map { path -> occurrence(path.pointer, path.path,
                "Nested resource type is not exposed as a root resource") }
        : count(distinct(api.paths.map { path ->
                count(pathSegments(path.path)) > 0 ? pathSegments(path.path)[0] : null })) > rule.parameters["maximum"]
        ? [occurrence("/paths", "API",
            "API has "
              + ("" + count(distinct(api.paths.map { path ->
                    count(pathSegments(path.path)) > 0 ? pathSegments(path.path)[0] : null })))
