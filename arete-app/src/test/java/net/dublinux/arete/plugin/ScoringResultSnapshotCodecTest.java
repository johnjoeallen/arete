...
class ScoringResultSnapshotCodecTest {

    private final ObjectMapper mapper = new ObjectMapper();

    @Test
    void gradeAndPassingScoreSurviveARoundTrip() throws Exception {
        AggregatedScoringResult original = new AggregatedScoringResult(
                List.of(), List.of(), 111, 92.5, 92.5, "B-", 90.0);

        String json = ScoringResultSnapshotCodec.toJson(mapper, original, List.of("generic-policy"));
        AggregatedScoringResult reloaded = ScoringResultSnapshotCodec.fromJson(mapper, json).result();
